...
npm.cmd run lint: 통과
npm.cmd test: 125/125 통과 (신규 29건 포함)
npm.cmd run check:browser: 3뷰포트 전 항목 통과 (코칭·접기 실상태 포함)
git diff --check: 통과
codex exec 감사: 1차 FAIL 10건 → 2차 FAIL 3건 → 3차 FAIL 3건(high 1) → 4차 FAIL 1건 → 전부 수정 후 5차 진행
```

감사를 반복한 값이 컸다. 2~4차 지적 7건 중 5건이 **앞 라운드의 내 수정이 만든 것**이다(세대 캡처 시점, 발화 소유권 취소, 크기 추정 상한 2회, 낡은 문서 저장). 동시성·한도 수정은 한 번에 맞히기 어려우므로, 이 코드를 다시 손대는 사람은 수정 뒤 반드시 재감사를 돌리는 것을 권한다.

특히 크기 추정은 두 라운드 연속으로 "상한이 아니다"를 지적받았다. 고정 상수로 추정하려던 접근 자체가 틀렸고, 검증기가 형태를 좁히지 않는 한 어떤 상수도 상한이 될 수 없었다. 4차에서 검증 단계 정규화로 옮기고 나서야 해결됐다.
