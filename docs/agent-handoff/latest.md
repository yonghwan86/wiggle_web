...
5. 칭찬 필터를 `잘하-`까지 넓혀 `잘 할 수 있어` 같은 격려 문구도 거부된다. 또 지시어 없이도 `그림을 좋아해`류를 막으므로, 교사 초안의 정상 관찰 문장 `아이가 그림을 좋아해요`도 함께 거부된다(관형형 `그림을 좋아하는 것 같아요`는 통과). 아동 안전 불변식을 지키려고 의도적으로 차단 쪽으로 기울인 선택이니, 운영에서 AI 거부율(`AI_RESPONSE_INVALID` 502)을 지켜보고 과차단이 잦으면 교사 초안 경로만 완화하는 것을 검토한다.

### 이 필터를 다시 손댈 사람에게

칭찬 필터는 이 세션에서 일곱 번 고쳤고, 여섯 번은 "빠진 표현을 추가"하는 방식이었다가 매번 다음 반례가 나왔다. 문서 크기 한도도 똑같이 네 라운드를 반복했다. **열거로 막으려 하면 끝나지 않는다**는 것이 공통 교훈이다. 지금 구조는 이렇다.

1. **막을 것이 아니라 통과시킬 것을 열거한다.** `좋아` 뒤에 허용하는 관형·연결형(`하는/하던/하면/하고/하지/하게/해서`)만 적고 나머지는 전부 막는다. 새 시제·존댓말이 등장해도 기본값이 차단이다.
2. **전처리는 원문을 지우지 않는다.** 의문형을 판별하려고 어미 구간을 치환했더니 그 안에 낀 금지어(`좋아칭찬?`)까지 검사 전에 사라졌다. 지금은 물음표만 낱말로 바꿔 표시를 남긴다.
3. **양방향으로 테스트한다.** 누출만 막는 테스트를 쓰면 과차단이 안 보이고, 반대도 마찬가지다. 차단 20종·허용 15종을 필터 단위와 두 최종 validator 양쪽에서 고정했다. 표현을 추가할 때는 반드시 허용 목록도 함께 늘린다.
