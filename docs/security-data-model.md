...
- 그림 비밀번호와 교사 PIN은 개인 salt를 둔 PBKDF2-SHA256 120,000회 결과만 저장한다. Worker에서는 `nodejs_compat`의 비동기 `node:crypto` 구현을 사용하며, 새 그림 비밀번호는 반복 가능한 그림 세 개이고 기존 네 개 비밀번호도 그대로 검증한다.
