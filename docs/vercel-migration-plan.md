...
5. **교사 인증 교체** ✅ 완료(2026-08-19 실왕복까지): 구글 OAuth 코드 플로우(PKCE S256, state 상수시간 비교, 미검증 이메일 거부)를 자체 구현해 기존 teacher_sessions 재사용 — 새 패키지 0개. SIWC(oai-* 헤더) 신뢰 경로는 Sites 밖에서 헤더 위조로 임의 교사 로그인이 가능해 완전 제거. 게이트: 단위 테스트 7종 + security-regressions 재작성 + 전 스위트·browser-check 통과 + env 미설정 시 503 fail-closed 실측. 실왕복 실측(2026-08-19, 사용자 OAuth 클라이언트 `wiggle` 프로젝트): 구글이 redirect_uri를 수락, 위조 state·코드 없는 콜백은 세션 미발급으로 랜딩 복귀, 실제 동의 후 자격증명 없는 실이메일 교사 계정 + 유효 세션 생성 확인.
6. **Vercel 연결** 진행 중(2026-08-19, 데이터 이전 없음): 사용자 완료 — repo import(프로젝트 `wiggle-web`, main 자동 배포), env 9종 입력(전부 Sensitive), Deployment Protection 해제(공개 접근). **첫 배포는 전 요청 500** — 원인은 `package.json`의 `"type": "module"`(Workers 스택 잔재). Vercel 함수 런처(`___next_launcher.cjs`)가 빌드 산출물 `.js`를 `require()`로 여는데 이 선언이 산출물을 ESM으로 선언해 ERR_REQUIRE_ESM으로 즉사(로컬 Node 24는 require(ESM)을 지원해 재현 안 됨 — 런타임 로그로 확진). 루트에 평문 `.js`가 없어(전부 .mjs/.ts) 선언 제거만으로 해결. 남은 것: 재배포 후 전 경로 실측, 구글 리디렉션 URI에 배포 도메인 추가, 커스텀 도메인(wiggleweb.app — 미등록 상태, 구매 필요) 연결.
...
- ~~구글 로그인 실왕복~~ ✅ 해소(2026-08-19): 로컬 3001에서 전 과정 실측 통과(5단계 기록 참조). 등록된 리디렉션 URI는 localhost 3000·3001 두 개 — **배포 도메인용 URI는 도메인 확정 후 추가해야 교사 로그인이 된다.**
- `wiggleweb.app` 커스텀 도메인이 Vercel에 추가만 되고 **아직 등록(구매)되지 않음** — 전 세계 DNS에서 NXDOMAIN. 구매 전까지 공개 주소는 `wiggle-web-yonghwan86s-projects.vercel.app`(보호 해제 상태). 도메인이 붙으면 Deployment Protection을 Standard로 되돌려도 된다(커스텀 프로덕션 도메인은 보호와 무관하게 공개).
