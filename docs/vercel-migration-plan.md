...
6. **Vercel 연결** ✅ 완료(2026-08-19, 데이터 이전 없음): repo import(프로젝트 `wiggle-web`, main 자동 배포)·env 9종(전부 Sensitive)·보호 해제·서울 리전(icn1), 공개 주소 `https://wiggleweb.vercel.app`(무료 서브도메인 — `wiggle-web.vercel.app`은 타인 선점이라 사용 금지). **운영에서만 드러난 버그 3건을 실측으로 확진·수정**:
   - `"type": "module"`(Workers 잔재) → Vercel 함수 런처(`___next_launcher.cjs`)의 require가 산출물 `.js`를 ESM으로 오인, 전 요청 ERR_REQUIRE_ESM 500. 로컬 Node 24는 require(ESM)을 지원해 재현 불가, 런타임 로그로 확진. 루트에 평문 `.js`가 없어 선언 제거로 해결(50a7965).
