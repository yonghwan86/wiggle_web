...
export function StudentHome() {
  ...
  return <main className="app-shell student-app">
    ...
    {newestMessage && <aside className="teacher-message"><span>👩‍🏫 선생님</span><p>{newestMessage.body}</p><SpeakButton text={`선생님이 말했어요. ${newestMessage.body}`} compact /><small>{newestMessage.audience === "all" ? "우리 반에게" : "나에게"}</small></aside>}
    <section className="welcome curriculum-welcome"><div className="welcome-main"><div className="welcome-title-row"><span className="welcome-activity-emoji" aria-hidden="true">{teacherLesson?.emoji ?? "✨"}</span><div><p className="eyebrow">오늘 선생님 추천</p><h1>{data.currentActivityLabel}</h1><p>{teacherLesson?.description ?? "빈 도화지에서 시작하고, 필요할 때만 그리미를 불러요."}</p></div><SpeakButton text={`오늘은 ${data.currentActivityLabel}. ${teacherLesson?.description ?? "빈 도화지에서 시작하고, 필요할 때만 그리미를 불러요."} 파란 시작 버튼을 눌러요.`} /></div><a className="button primary child-primary-action" href={teacherActivityPath}><span aria-hidden="true">▶️</span>오늘 활동 시작</a></div><div className="curriculum-total"><strong>{completed.size}/30</strong><span>활동 완성</span><div aria-label={`전체 진행률 ${progress}%`}><i style={{ width: `${progress}%` }} /></div><small>내 다음 추천</small><a href={`/student/draw/new?lesson=${recommended.slug}`}>{recommended.emoji} {recommended.title} →</a></div></section>
    <section><div className="section-title"><div><h2>내 그림 여행 4단계</h2><p>순서는 추천이에요. 원하는 단계부터 시작해도 좋아요.</p></div><a href="/student/archive">성장 기록 →</a></div><div className="curriculum-grid">{CURRICULUM_STAGES.map((stage) => {
      ...
      return <a className={`curriculum-card stage-${stage.stage}`} href={stage.path} key={stage.stage}><span>{stage.emoji}</span><div><small>{stage.stage}단계 · 잠금 없음</small><h3>{stage.title}</h3><p>{stage.description}</p><div className="stage-progress"><i style={{ width: `${stage.stage === 4 ? Math.min(100, done * 20) : done * 10}%` }} /></div><b>{stage.stage === 4 ? `${done}개 자유 작품` : `${done}/10 완성`}</b></div><strong>▶️ 열기</strong></a>;
