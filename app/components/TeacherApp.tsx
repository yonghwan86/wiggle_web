...
export function TeacherApp({ classroomId = "" }: { classroomId?: string }) {
  ...
  // 재발급은 복구 해시를 교체하고 기존 세션을 끊는다. 연속 클릭으로 두 번 돌면 화면에는
  // 이미 무효가 된 앞 토큰이 남을 수 있어, 학생이 그 카드로 복구하지 못한다.
  const [reissuing, setReissuing] = useState("");
  const reissuingRef = useRef("");
  ...
  const closePreview = useCallback(() => { viewingStudentIdRef.current = ""; setViewingStudentId(""); setDraftId(""); setDraftBody(""); setDraftSent(false); setDraftStudentId(""); setFamilySharePanelOpen(false); setGuardianConsentConfirmed(false); setConsentMethod(""); setPreviewMessageBody(""); setPreviewMessageStatus(""); setPreviewMessageError(""); setPreviewMessageSending(false); }, []);
  useEffect(() => {
  ...
  async function reissueRecovery(student: Student) {
  ...
  function openPreview(student: Student, openFamilyShare = false) {
    ...
    closePreview(); viewingStudentIdRef.current = student.id; setViewingStudentId(student.id); setFamilySharePanelOpen(openFamilyShare);
  }
