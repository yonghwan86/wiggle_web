...
export function TeacherApp({ classroomId = "" }: { classroomId?: string }) {
  ...
  useEffect(() => {
    // 학생이 목록에서 사라지면(삭제·학급 변경) 열린 미리보기도 닫는다.
    if (viewingStudentId && classroomData && !viewingStudent) setViewingStudentId("");
  }, [classroomData, viewingStudent, viewingStudentId]);
  const closePreview = useCallback(() => { setViewingStudentId(""); setDraftId(""); setDraftBody(""); setDraftSent(false); setGuardianConsentConfirmed(false); setConsentMethod(""); }, []);
  ...
  async function archiveStudent(student: Student) {
    ...
    if (viewingStudentId === student.id) setViewingStudentId("");
  ...
  async function requestTeacherDraft(student: Student) {
    if (!classroomData || !student.artworkId || draftLoading) return; setViewingStudentId(student.id); setDraftLoading(true); setDraftId(""); setDraftBody(""); setDraftSent(false); setError("");
